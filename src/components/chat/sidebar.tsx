...
export function SideBar(): React.JSX.Element {
  ...
  return (
    <>
      ...
      {showMobileOverlay && (
        <div
          className="bg-overlay fixed inset-0 z-40 backdrop-blur-sm md:hidden animate-in fade-in duration-700"
          aria-hidden="true"
      ...
      <div
        ...
        className={cn(
          'bg-sidebar text-sidebar-foreground border-sidebar-border fixed top-0 left-0 z-50 flex h-svh w-[260px] flex-col border-r shadow-xl transition-all duration-800 ease-in-out origin-left data-[state=closed]:-translate-x-full data-[state=closed]:opacity-0 data-[state=closed]:-rotate-6 data-[state=open]:translate-x-0 data-[state=open]:opacity-100 data-[state=open]:rotate-0 motion-reduce:transition-none',
          'md:static md:h-full md:translate-x-0 md:opacity-100 md:rotate-0 md:transition-none md:shadow-none md:data-[state=closed]:hidden'
        )}
      >
        <div className="flex h-full flex-col px-4 pt-[calc(1rem+env(safe-area-inset-top))] pb-[calc(1rem+env(safe-area-inset-bottom))]">
          ...
          <div className="mb-4 flex items-center justify-between">
            <Link
              href="/"
              className="group focus-visible:ring-ring/50 focus-visible:ring-offset-background flex items-center rounded-md focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none"
            >
              <span className="bg-gradient-to-r from-primary via-primary/80 to-primary/60 bg-clip-text text-transparent group-hover:from-primary group-hover:via-primary group-hover:to-primary/80 font-display text-3xl font-bold tracking-tight transition-all duration-200">
                DevBot
              </span>
            </Link>
            <Button
          ...
          {/* Persona Store Button & Themes Button */}
          <div className="mt-auto flex flex-col gap-2 pt-4">
            ...
            >
              <span className="bg-primary/5 pointer-events-none absolute inset-0 translate-y-full transition-transform duration-200 ease-out group-hover:translate-y-0" />
              <Palette className="text-primary/60 group-hover:text-primary relative mr-2.5 size-4 transition-colors duration-200" />
              <span className="relative text-sm font-medium">Themes</span>
              <span
                className="text-primary/30 group-hover:text-primary/50 relative ml-auto font-serif text-xs transition-colors duration-200"
                aria-hidden="true"
              >
      ...
      <Dialog open={isThemeModalOpen} onOpenChange={setIsThemeModalOpen}>
        <DialogContent className="max-w-md top-24">
          <DialogHeader>
